...
const match = pkg.repository.url.match(/github\.com[/:](?<ownerRepo>[^/]+\/[^/.]+?)(?:\.git)?$/);
if (!match) {
  throw new Error(`Could not extract GitHub owner/repo from repository URL: ${pkg.repository.url}`);
}
...
if (revert) {
  ...
  readme = readme.replace(
    new RegExp(`!\\[(?<alt>[^\\]]*)\\]\\(${escapeRegExp(rawTagUrl)}(?<url>[^)]+)\\)`, "g"),
    "![$<alt>]($<url>)",
  );
  // Revert absolute blob URLs → relative for links
  readme = readme.replace(
    new RegExp(`\\[(?<text>[^\\]]+)\\]\\(${escapeRegExp(blobBaseUrl)}(?<url>[^)]+)\\)`, "g"),
    "[$<text>]($<url>)",
  );

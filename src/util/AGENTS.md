...
- [input.ts](input.ts) - SVG input loading, validation, and lazy image data. Core constants: `inputImageSize = 108`, `inputContentSize = 72`, `inputImageMargin = 18`.
...
**Cache-Aware Output** ([output.ts](output.ts)): Every `generateFile()` / `generatePngs()` call checks `cache.isUpToDate()` before generating. If up-to-date, the file is skipped and not yielded. After writing, the hash is recorded via `cache.recordBuffer()`.

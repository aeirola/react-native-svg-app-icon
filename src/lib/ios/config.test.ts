...
describe("ios/config", () => {
	describe("getConfig", () => {
		...
		it("resolves relative iosOutputPath against projectRoot", async ({
			tmpDir,
		}) => {
			const config = await getConfig(
				{
			...
			expect(config.iosOutputPath).toBe(
				path.join(tmpDir, "ios", "App", "Images.xcassets"),
			);
		});
		...
		it("prefers directory matching app name over other matches", async ({
			...
			const config = await getConfig(
				{ appName: "My App", projectRoot: tmpDir },
				undefined,
			);
		...
		it("throws error when Images.xcassets directory does not exist", async ({
			...
			await expect(
				getConfig({ projectRoot: tmpDir }, undefined),

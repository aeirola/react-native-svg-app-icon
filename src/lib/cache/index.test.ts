...
describe("cache", () => {
  describe("CacheSession", () => {
    describe("isUpToDate", () => {
      it("returns false when no cache exists (cold start)", async ({ tmpDir }) => {
      ...
      it("returns false when force is true even if cache is warm", async ({ tmpDir }) => {
        const inputBuffer = Buffer.from("<svg>input</svg>", "utf-8");
        const outputFile = await writeFile("output.png", "fake-png-data");
      ...
      it("returns true when inputs and output are unchanged (warm cache)", async ({ tmpDir }) => {
        const inputBuffer = Buffer.from("<svg>input</svg>", "utf-8");
        const outputFile = await writeFile("output.png", "fake-png-data");
      ...
      it("returns false for all outputs when an input file changes", async ({ tmpDir }) => {
      ...
      it("returns false for all outputs when package version has changed", async ({ tmpDir }) => {
        const inputBuffer = Buffer.from("<svg>input</svg>", "utf-8");
        const outputFile = await writeFile("output.png", "fake-png-data");
      ...
      it("returns false only for a deleted output, true for unchanged ones", async ({ tmpDir }) => {
        const inputBuffer = Buffer.from("<svg>input</svg>", "utf-8");
        const output1 = await writeFile("out1.png", "png-data-1");
        const output2 = await writeFile("out2.png", "png-data-2");
      ...
      it("returns false for an output that was modified externally", async ({ tmpDir }) => {
        const inputBuffer = Buffer.from("<svg>input</svg>", "utf-8");
      ...
      it("handles multiple input files — change to any input invalidates all outputs", async ({
        ...
        const bgBufferChanged = Buffer.from("<svg>background-changed</svg>", "utf-8");

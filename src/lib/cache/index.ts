...
export class CacheSession {
  ...
  async isUpToDate(outputFilePath: string): Promise<boolean> {
    ...
    if (await this.hasPackageVersionChanged()) {
      this.logger?.debug(`Cache miss for ${outputFilePath}: package version changed`);
      return false;
    }

    if (await this.haveInputsChanged()) {
      this.logger?.debug(`Cache miss for ${outputFilePath}: input files changed`);
      return false;
    }
    ...
    if (!cachedOutputHash) {
      this.logger?.debug(`Cache miss for ${outputFilePath}: no cached hash found`);
      return false;
    }
    ...
    this.logger?.debug(`Cache miss for ${outputFilePath}: output file changed on disk`);
    return false;
  }
  ...
  private hasPackageVersionChanged(): Promise<boolean> {
    this.packageVersionChangedPromise ??= Promise.all([this.loadCache(), getPackageVersion()]).then(
  ...
  private computeInputHashes(): Record<string, string> {
    this.cachedInputHashes ??= Object.fromEntries(
      Object.entries(this.inputFileBuffers).map(([role, buffer]) => [role, hashBuffer(buffer)]),
    );

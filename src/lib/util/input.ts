...
type ResolvedConfig = Required<Pick<InputConfig, "backgroundPath" | "foregroundPath">>;
...
export async function readIcon(
  ...
  const [backgroundBuffer, foregroundBuffer] = await Promise.all([
    fse.readFile(fullConfig.backgroundPath).catch((error) => {
      throw new Error(`Failed to read background icon at ${fullConfig.backgroundPath}`, {
        cause: error,
      });
    }),
    fse.readFile(fullConfig.foregroundPath).catch((error) => {
      throw new Error(`Icon is required, but not found at ${fullConfig.foregroundPath}`, {
        cause: error,
      });
    }),
  ]);
...
function getConfig(config: InputConfig, logger: Logger | undefined): ResolvedConfig {
  const foregroundPath = path.resolve(config.projectRoot, config.foregroundPath);

  let backgroundPath: string;
  if (config.backgroundPath !== undefined) {
    backgroundPath = path.resolve(config.projectRoot, config.backgroundPath);
  } else {
    logger?.debug("No background icon specified, falling back to white background");

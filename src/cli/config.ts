 ...
 */
const CliConfig = LibConfig.omit("appName", "projectRoot", "backgroundPath", "foregroundPath").and({
  backgroundPath: LibConfig.required().get("backgroundPath").default("./icon-background.svg"),
...
const configFlags: {
  ...
  platforms: ["--platforms <platforms...>", "platforms for which to generate icons"],
...
const { backgroundPath: defaultBackgroundPath, ...defaultConfig } = CliConfig.assert({});
 ...
 */
export async function resolveConfig(args: string[] = []): Promise<ResolvedConfig> {
